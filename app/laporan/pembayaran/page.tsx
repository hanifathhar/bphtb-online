...
export default function RekapPembayaranPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      params.append("jenis", "pembayaran");
      if (tahun) params.append("tahun", tahun);
      if (bulanAwal) params.append("bulanAwal", bulanAwal);
      if (bulanAkhir) params.append("bulanAkhir", bulanAkhir);
      if (statusFilter === "lunas") params.append("statusBayar", "1");
      if (statusFilter === "tunggakan") params.append("statusBayar", "0");
      ...
      console.error("Gagal mengambil data rekap pembayaran:", e);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchData();
  }, [tahun, bulanAwal, bulanAkhir, statusFilter]);

  useEffect(() => {
    setPage(1);
  }, [tahun, bulanAwal, bulanAkhir, statusFilter, search, limit]);
  ...
  const formatDateDDMMYYYY = (d: string | Date) => {
    if (!d) return "";
    const date = new Date(d);
    if (isNaN(date.getTime())) return "";
    const day = String(date.getDate()).padStart(2, "0");
  ...
  const totalDibayar = filteredData.reduce(
    (acc, curr) =>
      acc + (curr.statusBayar === 1 ? Number(curr.nilaiSudahDibayar || curr.bphtb || 0) : 0),
    0
  );
  const totalTunggakan = filteredData.reduce(
    (acc, curr) =>
      acc + (curr.statusBayar === 1 ? 0 : Number(curr.nilaiBelumDibayar ?? curr.bphtb ?? 0)),
    0
  );
  ...
  const handleExportExcel = () => {
    const exportRows = filteredData.map((item, index) => {
      ...
      const sudahDibayar = isLunas ? Number(item.nilaiSudahDibayar || ketetapanNominal) : 0;
      const tunggakan = isLunas ? 0 : Number(item.nilaiBelumDibayar ?? ketetapanNominal);
      const tglKetetapan = item.tglSkp
        ? formatDateYYYYMMDD(item.tglSkp)
        : item.tglBerkas
        ? formatDateYYYYMMDD(item.tglBerkas)
        : "-";
      const tglBayarFormatted = isLunas && item.tglBayar ? formatDateDDMMYYYY(item.tglBayar) : "";
      ...
      return {
        ...
        "Nomor STS": isLunas ? item.noSts || "-" : "",
        "Tanggal Pembayaran": tglBayarFormatted,
        "Denda (Rp)": 0,
        "Jumlah Dibayar (Rp)": sudahDibayar,
        "Tunggakan (Rp)": tunggakan,
        "Status Pembayaran": isLunas ? "Lunas" : "",
      };
    });

    const worksheet = XLSX.utils.json_to_sheet(exportRows);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Rekap_Penerimaan_BPHTB");
    XLSX.writeFile(
      workbook,
      `Daftar_Penerimaan_Pajak_BPHTB_${tahun}_Bulan_${bulanAwal}_sd_${bulanAkhir}.xlsx`
    );
  };
    ...
    <DashboardShell active="laporan">
      ...
      <div className="no-print space-y-4 border-b border-slate-200 pb-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 text-xs font-semibold mb-2">
              <CreditCard size={14} /> Daftar Surat Penerimaan Pajak BPHTB
            </div>
            <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight">
              Rekap Pembayaran & Penerimaan BPHTB
            </h1>
            <p className="text-xs sm:text-sm text-slate-500">
              Format baku laporan resmi Daftar Surat Penerimaan Pajak Daerah (BPHTB) Kabupaten Tapanuli Selatan.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            >
              <Printer size={16} /> Cetak Laporan (Landscape)
            </button>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 p-4 rounded-2xl shadow-xs grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 gap-3 text-xs">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 font-semibold focus:outline-none focus:ring-2 focus:ring-emerald-500"
            >
          ...
          {/* Status Bayar */}
          <div>
            <label className="block font-semibold text-slate-700 mb-1">Status Bayar</label>
            <select
              value={statusFilter}
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                placeholder="Cari Kohir, WP, STS..."
                className="w-full pl-8 pr-7 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 placeholder-slate-400 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500"
              />
              <Search
                size={14}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 text-xs">
          ...
          <div className="p-3.5 rounded-2xl bg-white border border-slate-200 shadow-xs">
            <p className="text-slate-500">Total Ketetapan Pajak</p>
          ...
          <div className="p-3.5 rounded-2xl bg-emerald-50/70 border border-emerald-200 shadow-xs">
            <p className="text-emerald-700 font-medium">Realisasi Penerimaan (Lunas)</p>
            <p className="text-lg font-black text-emerald-600 mt-0.5">{formatNumber(totalDibayar)}</p>
          </div>
          <div className="p-3.5 rounded-2xl bg-amber-50/70 border border-amber-200 shadow-xs">
            <p className="text-amber-800 font-medium">Total Tunggakan (Belum Bayar)</p>
        ...
        <div className="flex items-center justify-between gap-4 mb-3 pb-2 border-b border-black print:border-none">
          ...
          <div className="flex-1 text-center pr-14 sm:pr-16">
            ...
            <h1 className="text-[11px] sm:text-xs md:text-sm font-black tracking-wide text-black uppercase leading-tight mt-0.5">
              DAFTAR SURAT PENERIMAAN PAJAK BPHTB
            </h1>
              ...
              {loading ? (
                <tr>
                  <td colSpan={13} className="border border-black px-4 py-8 text-center text-slate-500">
                    Memuat data daftar penerimaan pajak BPHTB...
                  </td>
                </tr>
              ) : paginatedData.length === 0 ? (
                <tr>
                  <td colSpan={13} className="border border-black px-4 py-8 text-center text-slate-500">
                    Tidak ada data penerimaan pajak pada periode ini.
                  </td>
                </tr>
              ) : (
                paginatedData.map((item, index) => {
                  ...
                  const isLunas = item.statusBayar === 1;
                  const ketetapanNominal = Number(item.bphtb || 0);
                  const sudahDibayar = isLunas
                    ? Number(item.nilaiSudahDibayar || ketetapanNominal)
                    : 0;
                  const tunggakan = isLunas
                    ? 0
                    : Number(item.nilaiBelumDibayar ?? ketetapanNominal);
                  const tglKetetapan = item.tglSkp
                    ? formatDateYYYYMMDD(item.tglSkp)
                    : item.tglBerkas
                    ? formatDateYYYYMMDD(item.tglBerkas)
                    : "-";
                  const tglBayarFormatted =
                    isLunas && item.tglBayar ? formatDateDDMMYYYY(item.tglBayar) : "";
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="border border-black px-1 py-1 text-center align-top font-mono whitespace-nowrap">
                        {tglBayarFormatted}
                      </td>
