    ...
    <DashboardShell active="laporan">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 text-emerald-600 text-xs font-semibold mb-2">
            <CreditCard size={14} /> Pelaporan Penerimaan Kas
          </div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight">
            Rekap Pembayaran & Realisasi Penerimaan
          </h1>
          <p className="text-xs sm:text-sm text-slate-500">
            Laporan seluruh transaksi BPHTB yang telah lunas disetorkan ke Kas Daerah melalui Bank Persepsi.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Printer size={15} /> Cetak Rekap Penerimaan
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="p-4 rounded-2xl bg-white border border-slate-200">
          <p className="text-xs text-slate-500 font-medium">Jumlah Transaksi Lunas</p>
          <p className="text-2xl font-bold text-slate-900 mt-1">{data.length} Transaksi</p>
        </div>
        <div className="p-4 rounded-2xl bg-white border border-slate-200">
          <p className="text-xs text-slate-500 font-medium">Total Realisasi Penerimaan Kas Daerah</p>
          <p className="text-2xl font-black text-emerald-600 mt-1">{formatRupiah(summary.totalSudahBayar || 0)}</p>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-700">
            <thead className="bg-slate-50 text-slate-500 uppercase tracking-wider font-semibold border-b border-slate-200">
              <tr>
                <th className="px-4 py-3 rounded-l-xl">No. Bukti & Bank</th>
                <th className="px-4 py-3">No. Kohir / Berkas</th>
                <th className="px-4 py-3">Nama Wajib Pajak</th>
                <th className="px-4 py-3">Tanggal Pelunasan</th>
                <th className="px-4 py-3 text-right rounded-r-xl">Setoran Masuk (Rp)</th>
              </tr>
            </thead>
              ...
              {loading ? (
                <tr>
                  <td colSpan={5} className="px-4 py-12 text-center text-slate-500">
                    Memuat data realisasi pembayaran...
                  </td>
                </tr>
              ) : data.length === 0 ? (
                <tr>
                ...
                paginatedData.map((item) => (
                  <tr key={item.idBerkas} className="hover:bg-slate-100 transition">
                    <td className="px-4 py-3.5">
                      <p className="font-bold text-red-700 font-mono">{item.noBuktiBayar || "-"}</p>
                      <p className="text-[11px] text-slate-500">{item.bankBayar || "Bank BPD"}</p>
                    </td>
                    <td className="px-4 py-3.5">
                    ...
                    <td className="px-4 py-3.5 text-slate-700">
                      {item.tglBayar ? new Date(item.tglBayar).toLocaleDateString("id-ID") : "-"}
                    </td>
                    <td className="px-4 py-3.5 text-right font-bold text-emerald-600 text-sm">
                      {formatRupiah(item.nilaiSudahDibayar || item.bphtb)}
                    </td>
                  </tr>
                ))
              )}
